...
history = pd.DataFrame(env.observer.renderer_history)
actions=list(history.action)
price=list(history.price)[-1]

#Save wallet state at time point equals 2
performance = pd.DataFrame.from_dict(env.action_scheme.portfolio.performance, orient='index')
starting_offset=1
if int(start_position['time offset'])>1:
    starting_offset=0
for i in range(starting_offset,len(performance)):
...
if action[0] != action[1]:
    act=pd.DataFrame.from_dict(env.action_scheme.portfolio.performance, orient='index')
    if action[0]==1:
